...
void CSiteCommandManager::guestUserCheck(CFCGIRequest* currRequest) const
{
    ...
    if(currRequest->getUser()->getCookie() == gs->getStringParamValue("guestCookieValue", "GUEST"))
    {
        ...
        currRequest->setUser(CUser::createNewUser(userKey, valueToString(currRequest->getRequest()->param.get("REMOTE_ADDR","")), valueToString(currRequest->getRequest()->param.get("HTTP_USER_AGENT",""))));
    }
}
