...
float CMySQLDBRequestResult::getFloatValue(const int rowIndex, const int fieldIndex, const float defValue) const
{
	...
	try
	{
		if (sqlRows[rowIndex][fieldIndex]) value = std::stof(sqlRows[rowIndex][fieldIndex]);
	}
	catch (...)
	{
		value = defValue;
...
double CMySQLDBRequestResult::getDoubleValue(const int rowIndex, const int fieldIndex, const double defValue) const
{
	if (rowIndex >= getRowsCnt()) 	  return defValue;
	if (fieldIndex >= getFiledsCnt()) return defValue;
	...
	try
	{
		if (sqlRows[rowIndex][fieldIndex]) value = std::stod(sqlRows[rowIndex][fieldIndex]);
	}
	catch (...)
	{
		value = defValue;
