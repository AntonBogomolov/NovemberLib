...
{
    if(fileMimeType.find("image/") != std::string::npos) return "image";
    if(fileMimeType.find("audio/") != std::string::npos) return "audio";
    if(fileMimeType.find("text/")  != std::string::npos) return "text";
    if(fileMimeType.find("video/") != std::string::npos) return "video";
    if(fileMimeType.find("application/") != std::string::npos) return "application";
    if(fileMimeType.find("model/") != std::string::npos) return "model";
    if(fileMimeType.find("chemical/") != std::string::npos) return "chemical";
    if(fileMimeType.find("windows/") != std::string::npos) return "windows";
    if(fileMimeType.find("x-world/") != std::string::npos) return "x-world";
    if(fileMimeType.find("message/") != std::string::npos) return "message";
    if(fileMimeType.find("xgl/") != std::string::npos) return "xgl";

    return "none";
}

const std::string MIMEType::getFileExt(const std::string& fileMimeType)
{
    // TEXT
    if(fileMimeType == "text/css") 	return "css";
    if(fileMimeType == "text/html") 	return "html";
    if(fileMimeType == "text/plain")	return "txt";
    if(fileMimeType == "text/javascript") return "js";
    if(fileMimeType == "text/xml") 	return "xml";

    // IMAGES
    if(fileMimeType == "image/gif") 	return "gif";
    if(fileMimeType == "image/jpeg") 	return "jpeg";
    if(fileMimeType == "image/pjpeg")	return "jpeg";
    if(fileMimeType == "image/png") 	return "png";
    if(fileMimeType == "image/svg+xml") return "svg";
    if(fileMimeType == "image/vnd.microsoft.icon") return "ico";
    // APP
    if(fileMimeType == "application/pdf") 		return "pdf";
    if(fileMimeType == "application/zip") 		return "zip";
    if(fileMimeType == "application/x-bittorrent")	return "torrent";
    if(fileMimeType == "application/vnd.ms-excel") 	return "xls";
    if(fileMimeType == "application/wps-office.xls")    return "xls";
    if(fileMimeType == "application/msword") 		return "doc";
    if(fileMimeType == "application/x-rar-compressed")	return "rar";
    if(fileMimeType == "application/x-zip-compressed")	return "zip";
    if(fileMimeType == "application/octet-stream")	return "rar";
    if(fileMimeType == "application/x-shockwave-flash")	return "swf";
    // AUDIO
    if(fileMimeType == "audio/mp4") 	return "mp4";
    if(fileMimeType == "audio/mpeg") 	return "mp3";
    if(fileMimeType == "audio/mp3") 	return "mp3";
    if(fileMimeType == "audio/ogg")	return "ogg";
    if(fileMimeType == "audio/x-ms-wma")return "wma";
    if(fileMimeType == "audio/vnd.wave")return "wav";
    if(fileMimeType == "audio/wav")	return "wav";
    // VIDEO
    if(fileMimeType == "video/mpeg") 	return "mpeg";
    if(fileMimeType == "video/avi") 	return "avi";
    if(fileMimeType == "video/x-msvideo")return "avi";
    if(fileMimeType == "video/webm") 	return "webm";
    if(fileMimeType == "video/mp4") 	return "mp4";
    if(fileMimeType == "video/ogg")	return "ogg";
    if(fileMimeType == "video/x-flv") 	return "flv";
    if(fileMimeType == "video/x-ms-wmv")return "wmv";

    for(auto it = exts.begin(); it != exts.end(); ++it)
    {
        std::string currMIME = it->second;
        if(currMIME == fileMimeType) return it->first;
    }
